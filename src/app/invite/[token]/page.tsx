...
import {
  invitePreviewCardClass,
  invitePreviewPageBackgroundClass,
  invitePreviewThemeVars,
} from '@/lib/invite-preview-theme';
...
export default async function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!parsed.valid) {
    ...
    return (
      <main
        className={cn('flex min-h-screen flex-col items-center justify-center bg-slate-50 p-6')}
      >
  ...
  const pageBg = invitePreviewPageBackgroundClass(preview.category_type);
  const cardShell = cn(
    'rounded-2xl border-2 p-8 shadow-lg backdrop-blur-[1px]',
    invitePreviewCardClass(preview.category_type),
  );
  const themeVars = invitePreviewThemeVars(preview.category_type);

  return (
    <main className={cn('flex min-h-screen flex-col items-center justify-center p-6', pageBg)}>
      <div className="w-full max-w-md space-y-6" style={themeVars}>
        <div className={cardShell}>
          <div className="text-center">
            ...
            <h1 className="mt-3 text-2xl font-bold leading-snug text-foreground">
              <span style={{ color: 'var(--invite-accent)' }}>{preview.host_name}</span> invited you
              to join{' '}
