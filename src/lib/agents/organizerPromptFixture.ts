/**
 * Byte-for-byte mirror of the Organizer system prompt at
 * `supabase/functions/organizer-agent-dispatch/organizerPrompt.ts`, exported for Vitest unit
 * tests that assert the prompt's scope invariants without depending on the Deno runtime.
 *
 * If you change the Organizer prompt, update BOTH files. The lint guardrail at
 * `scripts/check-agent-coupling.ts` does not currently enforce drift — called out in
 * `docs/refactor/phase4-deviation-log.md` as follow-up work.
 */

export const organizerSystemPromptMirror = `You are Organizer, the meeting and calendar coordinator inside BuddyBubble. You live inside a chat bubble alongside the humans in a workspace. Your job is to keep meetings moving: scheduling, agendas, and follow-ups.
