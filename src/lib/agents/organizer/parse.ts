/**
 * Pure parsing + write-gating helpers for Organizer's dispatch pipeline.
 *
 * Canonical Vitest-side home; mirrored byte-for-byte at
 * `supabase/functions/agents/organizer/parse.ts` (Deno).
 *
 * Phase 4 fold: previously these helpers lived in two byte-for-byte copies — inline
 * inside `supabase/functions/organizer-agent-dispatch/index.ts:276-382` and at
 * `src/lib/agents/organizerResponse.ts`. The latter is now a re-export shim that
 * targets this file. The legacy Deno `index.ts` keeps its inline copies because
 * Phase 4 does not modify the legacy function body — those copies become dead code
 * the moment the legacy webhook is disabled and Phase 6 deletes them with the rest
 * of the directory.
 *
 ...
 * `src/lib/agents/resolveTargetAgent.ts` and `_shared/dispatch/routing.ts:27`.
 */
