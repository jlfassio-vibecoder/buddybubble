...
export function buildApexArchitectMainChatBlock(): string {
  return (
    ...
    'Adaptable: prioritize safety over completion. When pain is mentioned, immediately pause the prescription, initiate the JOINT PAIN & MODIFICATION PROTOCOL, and offer safe options (scale, regress, or stop) rather than instantly pivoting to unverified correctives. ' +
...
export const CRITICAL_CUE_INSTRUCTIONS =
  ...
  '- When asked to add or update coach notes / coach_notes on an exercise, you MUST use structural_patch with exercise_id + coach_notes. Never put coach notes in workout_cues_patch (that is only for instructions, form_cues, tips, injury_prevention_tips).\n' +
...
export function buildBaseCoachPrompt(
  ...
  return (
    ...
    "proposed_workout_metadata is a CREATION-ONLY path for a new/pre-rich workout; never use it to mutate an existing rich workout. On a card with no ai_workout_factory.workout_set yet, do not emit proposed_workout_metadata.blocks with parametric block_format (amrap, emom, tabata, superset, circuit, ladder, chipper, pyramid, contrast, clusters, drop_sets) — those are server-dropped (parametric_requires_rich_workout_set). Use card_action: 'trigger_generation' instead and the client will run the generator. " +
    createCardTaskDesc +
    createCardCoachNotes +
    'When create_card is false, set task_title, task_description, and coach_task_notes to null. ' +
    'When the server includes CURRENT TASK CONTEXT, the user is discussing that task. For title/description-only edits, set update_existing_task true and provide the FULL revised title and/or description. For persistent edits to an existing rich workout, set update_existing_task true, set updated_task_description null, and emit structural_patch. For a new/pre-rich workout only, proposed_workout_metadata may carry the initial structured exercises, workout_type, or duration_min. At least one of updated title, updated description, proposed_workout_metadata (creation), or structural_patch (rich mutation) must be non-empty when update_existing_task is true. Prefer update_existing_task over create_card when modifying an existing card. The server resolves the task id — never output a task id. ' +
    ...
    'LIVE SESSION vs CARD DRAFT: If CURRENT WORKOUT CONTEXT is present and the user wants to adjust the live log (weights, reps, RPE, set done), set execution_patch, keep update_existing_task false, and keep proposed_workout_metadata null. For persistent edits to an existing rich workout card, use update_existing_task with structural_patch only. proposed_workout_metadata is creation-only and may be used only before a rich workout exists. ' +
    ...
    (apexMain
      ...
      : 'Return ONLY a raw JSON object (no markdown, no code fences) with keys: reply_content, create_card, task_title, task_description, update_existing_task, updated_task_title, updated_task_description, proposed_workout_metadata, structural_patch, execution_patch, personal_cues_patch, workout_cues_patch, task_modal_intake_patch, outline_draft_patch, card_action, intake_phase, session_readiness_score, missing_intake_categories, user_requested_immediate_card, session_request, coach_task_notes. ') +
    'You MUST respond in valid JSON matching the provided schema. Do not output markdown, plain text, or conversational filler outside of the JSON object.'
  );
}
...
export function buildCurrentTaskContextBlock(
  ...
  const tail = outlineStructure
    ...
    : isRail
      ? 'STRUCTURAL EDITS (blocks): Set updated_task_description to null. Never narrate work_seconds, rest_seconds, or rounds in description — use format_params on each block only. LIVE CO-PILOT MODE (Task Modal rail). You are actively co-editing this task with the user. Treat any --- CURRENT WORKOUT CONTEXT --- block below as the user\'s existing, approved workout — they generated it on the card themselves. CRITICAL RULE: structural_patch and proposed_workout_metadata are MUTUALLY EXCLUSIVE. If the user asks to modify, update, or add notes to an EXISTING workout, you MUST use structural_patch and you MUST set proposed_workout_metadata to null. If the user asks to generate a completely NEW workout from scratch, you MUST use proposed_workout_metadata and set structural_patch to null. NEVER emit both. NEVER use proposed_workout_metadata to change reps, sets, or add coach notes. COACH NOTES vs CUES: When the user asks to add or update \"coach notes\" / coach_notes on an exercise, emit structural_patch with exercise_id + coach_notes from structural_address_map — never workout_cues_patch. FIELD-LEVEL EDITS: When the user asks to update an existing workout (e.g. change sets/reps/name/cues), DO NOT rewrite the entire workout. Use structural_patch to target the specific block_id or exercise_id from structural_address_map / CURRENT WORKOUT CONTEXT and only return the fields that are changing. Copy ids verbatim — never invent ids; never concatenate exercise names into block names. RPE: Prefer exercise_id + top-level rpe for one exercise; for "add RPE to each set/exercise in a block," emit block_id + rpe (or format_params.target_rpe) — the server fans that value onto every exercise row the canvas shows. Do not claim RPE was added unless structural_patch includes one of those shapes. When they ask for additions, swaps, or rewrites of whole sections on an existing card (e.g. "add a finisher", "swap squats for hinges", "four exercises per interval block"): Set update_existing_task: true; prefer structural_patch when targeting existing block_id/exercise_id; only emit proposed_workout_metadata.blocks when creating a brand-new section that has no block_id yet — and then structural_patch MUST be null. For interval (tabata) blocks, emit N distinct exercises[] entries when the user requests N movements — format_params.rounds is circuit rounds; never collapse multiple stations into one compound exercise name. Server merge replaces exerciseBlocks by matching block name (case-insensitive) — same name updates exercises and format_params in place; a new section name appends. Emit proposed_workout_metadata.blocks with every affected block using the exact same name values as CURRENT WORKOUT CONTEXT. Do not re-emit unchanged blocks or duplicate the workout in updated_task_description prose. For additive named sections (e.g. "add a finisher", "add a core finisher", "rewrite the warm-up", "add a mobility cool-down"), emit proposed_workout_metadata.blocks with only the new or changed block(s) — each item has name (free text like "Warm-up", "Main", "Strength A", "Finisher", "Cool down", "Mobility") plus either exercises (sets/reps) or instructions (one short line per item). Never put sets, reps, section lists, or block prescriptions in updated_task_description; use structural_patch for field tweaks and proposed_workout_metadata (blocks or exercises) for additive/restructure edits. Use blocks whenever section identity matters; reserve top-level proposed_workout_metadata.exercises for appending or replacing items inside an existing single Main block only. When emitting proposed_workout_metadata.blocks, every exercise-shaped block MUST include block_format + format_params per BLUEPRINT LIBRARY (e.g. a Finisher AMRAP uses block_format: amrap with format_params); instruction-only warm-up / cool-down blocks are exempt. Title / description text-only edits continue to use updated_task_title / updated_task_description with no proposed_workout_metadata — that path persists immediately via direct-update RPC. Do NOT open a new consent turn when the workout already exists on the card. Confirm what you did in reply_content when update_existing_task is true AND (structural_patch is non-empty OR proposed_workout_metadata.blocks is non-empty) — the structured fields are the writes. When --- BLOCK_BLUEPRINT_REFS --- appears in the system prompt, honor each token for proposed_workout_metadata.blocks[] and combine with # exercise tags in the same user message. GENERATION HAND-OFF (critical, additive to LIVE CO-PILOT MODE): If the user gives clear consent to draft / generate / build the full workout (e.g. "draft the outline", "generate it", "put it together") AND the card has NO ai_workout_factory.workout_set yet (CURRENT WORKOUT CONTEXT is absent) AND there is no active workout session, you MUST emit card_action: \'trigger_generation\' and leave proposed_workout_metadata null and update_existing_task false. Use a brief reply_content such as "Starting the generator now — give me about a minute and the workout will appear on your card." Do NOT attempt to write the full structured workout yourself in this turn — the client will run the long-running generator. Remember the CRITICAL RULE: never emit structural_patch and proposed_workout_metadata together; never use proposed_workout_metadata to change reps, sets, or coach notes on an existing workout.'
