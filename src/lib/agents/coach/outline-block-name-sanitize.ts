/**
 * Post-parse hard boundaries for outline block names (Sprint 3 / MAX_TOKENS guard).
 *
 * Vertex `maxLength` is a generation hint only; models can still emit runaway prose in
 * `name`. Clamp and drop verbose names before persist.
 *
 * Deno mirror: `supabase/functions/agents/coach/outline-block-name-sanitize.ts`.
 */

import type { BlockShapeDrop } from './block-blueprint-library';
