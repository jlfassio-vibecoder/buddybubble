/**
 * Pure Buddy response parser + Gemini candidate text extractor.
 *
 * Canonical Vitest-side home; mirrored byte-for-byte at
 * `supabase/functions/agents/buddy/parse.ts` (Deno).
 *
 * Phase 5 fold: previously these helpers lived inline inside
 * `supabase/functions/buddy-agent-dispatch/index.ts:581-723`. The legacy Deno
 * `index.ts` keeps its inline copy because Phase 5 does not modify the legacy
 * function body — that copy becomes dead code the moment the legacy webhook is
 * disabled and Phase 6 deletes the directory.
 *
 * Buddy's parser is intentionally more aggressive than Coach's or Organizer's:
 * the model is allowed to wrap JSON in prose or markdown fences, so the parser
 * walks every `{` candidate with a balanced-brace extraction. This file MUST stay
