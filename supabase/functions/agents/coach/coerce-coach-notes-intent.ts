/**
 * MIRROR FILE — canonical lives at `src/lib/agents/coach/coerce-coach-notes-intent.ts`.
 * Run `pnpm check:agent-mirror` to verify parity.
 */

import { messageRequestsCoachNotes } from './exercise-cue-request.ts';
import type { CoachGeminiJsonResponse } from './parse.ts';
import type { CoachStructuralPatchOp } from '../../_shared/workout-metadata/structural-patch-types.ts';
