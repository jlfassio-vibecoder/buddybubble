/**
 * MIRROR FILE — canonical lives at `src/lib/agents/_shared/workout-metadata/merge-coach-proposed-into-task-metadata.ts`.
 *
 * Body below is byte-for-byte identical to the canonical Vitest-side file (excluding
 * this header). Parity: `pnpm check:agent-mirror` (leading JSDoc is stripped before compare).
 */
