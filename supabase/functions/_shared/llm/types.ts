/**
 * Vertex AI Gemini types shared across the consolidated dispatcher.
 *
 * The shapes mirror the legacy Generative Language API bodies at
 * `supabase/functions/bubble-agent-dispatch/index.ts` (search for `geminiGenerateJson`)
 * so per-strategy `responseSchema` literals can be lifted unchanged into Phase 2 / 4 / 5.
 *
 * This module is pure types — no runtime, no Deno globals — so it is safe to import from
 * any Deno function file at deploy time and from `deno test`.
 */
