-- Phase 12.1: persist Coach `card_action` on agent reply metadata.
...
comment on function public.agent_create_card_and_reply(
  ...
  'Agent reply with optional Kanban task; optional p_execution_patch, p_personal_cues, p_task_modal_intake_patch, and p_card_action on reply metadata; main reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
...
begin
  ...
  values (
    ...
    (
      ...
      || case
        when p_card_action is not null
          and jsonb_typeof(p_card_action) = 'object'
          and p_card_action <> '{}'::jsonb
        then jsonb_build_object('card_action', p_card_action)
        else '{}'::jsonb
      end
      end
    ),
...
comment on function public.agent_insert_coach_workout_draft_reply(
  ...
  'Agent inserts coach_draft reply; optional execution_patch, personal_cues_patch, task_modal_intake_patch, and card_action in metadata; reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
...
begin
  ...
  if v_title is null and v_desc is null and v_new_meta is null and v_reply_meta = '{}'::jsonb then
    raise exception 'agent_update_task_and_reply: at least one of p_new_title, p_new_description, non-empty p_new_metadata, or p_card_action must be provided'
      using errcode = 'P0001';
  end if;
...
comment on function public.agent_update_task_and_reply(
  ...
  'Agent updates an existing task (title, description, optional shallow metadata merge, optional p_card_action on reply metadata) + inserts thread reply (propagates thread_subject_user_id from trigger); idempotent per trigger+agent; service_role only.';
