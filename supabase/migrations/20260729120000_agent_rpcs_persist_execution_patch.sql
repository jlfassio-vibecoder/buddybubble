-- Persist execution_patch on the initial agent reply INSERT (no follow-up UPDATE in Edge).
-- Adds p_execution_patch jsonb to agent_create_card_and_reply and agent_insert_coach_workout_draft_reply.
...
begin
  ...
  values (
    ...
    '[]'::jsonb,
    case
      when p_execution_patch is null then
        '{}'::jsonb
      when jsonb_typeof(p_execution_patch) = 'array' and jsonb_array_length(p_execution_patch) = 0 then
        '{}'::jsonb
      else
        jsonb_build_object('execution_patch', p_execution_patch)
    end
  )
...
comment on function public.agent_create_card_and_reply(
  uuid, uuid, uuid, uuid, text, boolean, text, text, text, text, text, jsonb
) is
  'Agent reply with optional Kanban task; p_thread_id = thread root; optional p_seed_task_comment_text; p_execution_patch on reply metadata; service_role only.';
...
-- ---------------------------------------------------------------------------
-- agent_insert_coach_workout_draft_reply: same p_execution_patch merge
-- ---------------------------------------------------------------------------
...
comment on function public.agent_insert_coach_workout_draft_reply(
  ...
  'Agent reply with coach_draft in messages.metadata; optional p_execution_patch on same row; does not update tasks. service_role only.';
