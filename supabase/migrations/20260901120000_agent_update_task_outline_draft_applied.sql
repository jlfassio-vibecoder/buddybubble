-- Extend agent_update_task_and_reply: persist outline_draft_applied on coach reply metadata
-- (Sprint 3 builder outline co-pilot patch tool).
...
begin
  ...
  if v_title is null and v_desc is null and v_new_meta is null and v_reply_meta = '{}'::jsonb then
    raise exception 'agent_update_task_and_reply: at least one of p_new_title, p_new_description, non-empty p_new_metadata, p_card_action, or p_outline_draft_applied must be provided'
      using errcode = 'P0001';
  end if;
...
comment on function public.agent_update_task_and_reply(
  ...
  'Agent updates an existing task (title, description, optional shallow metadata merge) + inserts thread reply with optional p_card_action and p_outline_draft_applied on reply metadata; idempotent per trigger+agent; service_role only.';
