-- Phase 12.1.1 — Fix `card_action` persistence in agent RPCs.
--
-- Bugs in the prior migration `20260827120000_agent_rpcs_persist_card_action.sql`:
--
-- 1. `agent_create_card_and_reply`: the `if p_card_action ... then ... end if;`
--    block was nested **inside** the `if p_task_modal_intake_patch ... then`
--    block. The `end if;` for the intake guard was missing, so `card_action`
--    was only merged into `v_meta` when an intake patch was also present.
--    Generation hand-off turns (card_action only, no intake) silently dropped
--    `card_action` and the client never received the trigger.
--
-- 2. `agent_insert_coach_workout_draft_reply`: the `case ... when ... else ... end`
--    chain for the reply metadata had a missing `end` after the intake clause,
--    so the `card_action` clause was syntactically swallowed inside the intake
--    `else` branch. Same observable effect — `card_action` never made it onto
--    the persisted reply metadata.
--
-- This migration **replaces** both functions with structurally independent
-- sibling checks for each optional payload (`execution_patch`, `personal_cues`,
-- `task_modal_intake_patch`, `card_action`). `agent_update_task_and_reply` was
-- already correct in `20260827120000_*` and is intentionally left untouched.
--
-- After apply: run `pnpm types:db` to regenerate Database types.
...
begin
  ...
  -- Compose reply metadata via independent sibling checks. Each optional payload
  -- merges into v_meta only when present and non-empty; none gate on another.
  v_meta := '{}'::jsonb;
...
comment on function public.agent_create_card_and_reply(
  uuid, uuid, uuid, uuid, text, boolean, text, text, text, text, text, jsonb, jsonb, jsonb, jsonb
) is
  'Agent reply with optional Kanban task; optional p_execution_patch, p_personal_cues, p_task_modal_intake_patch, and p_card_action on reply metadata (each merged independently); main reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
...
comment on function public.agent_insert_coach_workout_draft_reply(
  ...
  'Agent inserts coach_draft reply; optional execution_patch, personal_cues_patch, task_modal_intake_patch, and card_action in metadata (each merged independently); reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
