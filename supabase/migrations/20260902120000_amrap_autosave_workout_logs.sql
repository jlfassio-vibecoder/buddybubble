-- AMRAP auto-save: link participant rows to generated workout_log tasks on host finalize.

alter table public.amrap_participants
  add column if not exists workout_log_task_id uuid null references public.tasks (id) on delete set null;

comment on column public.amrap_participants.workout_log_task_id is
  'Completed workout_log task auto-created for this participant when the host runs amrap_finalize_session.';

-- ---------------------------------------------------------------------------
-- RPC: amrap_finalize_session (host-only, idempotent + bulk auto-save)
-- ---------------------------------------------------------------------------
...
begin
  ...
  returning s.duration_seconds, s.block_snapshot, s.live_session_id
  into v_duration_seconds, v_block_snapshot, v_live_session_id;
...
comment on function public.amrap_finalize_session(uuid, jsonb) is
  'Host-only: locks official leaderboard, then auto-creates per-participant workout_log tasks (rounds > 0) in the Workout Logs bubble. Idempotent.';
