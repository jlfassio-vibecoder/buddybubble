-- Phase 1: Tabata interval format — attach RPC, segment advance, reset/finalize fixes.

begin;

-- ---------------------------------------------------------------------------
-- 1) Extend live_sessions.interval_wrapper_kind to include tabata
-- ---------------------------------------------------------------------------
...
alter table public.live_sessions
  ...
  check (interval_wrapper_kind in ('none', 'amrap', 'amrap_minimal', 'tabata'));

comment on column public.live_sessions.interval_wrapper_kind is
  'Wrapper renderer: none, amrap, amrap_minimal, tabata.';

-- ---------------------------------------------------------------------------
-- 2) RPC: tabata_create_for_session
-- ---------------------------------------------------------------------------
...
comment on function public.tabata_create_for_session(uuid, jsonb, jsonb) is
  'Host-only: ensures one live_interval_sessions row (interval_type=tabata), upserts host participant, sets wrapper config.';

revoke all on function public.tabata_create_for_session(uuid, jsonb, jsonb) from public;
grant execute on function public.tabata_create_for_session(uuid, jsonb, jsonb) to authenticated;
grant execute on function public.tabata_create_for_session(uuid, jsonb, jsonb) to service_role;

-- ---------------------------------------------------------------------------
-- 3) RPC: interval_advance_segment
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_segment not in ('idle', 'work', 'rest', 'done') then
    raise exception 'invalid segment';
  end if;

  if v_segment <> 'idle' and nullif(trim(p_mechanics_state->>'segment_started_at'), '') is null then
  ...
  if nullif(trim(p_mechanics_state->>'segment_started_at'), '') is not null then
    begin
...
comment on function public.interval_advance_segment(uuid, jsonb) is
  'Host-only: durable checkpoint for segmented interval position (tabata/emom).';

revoke all on function public.interval_advance_segment(uuid, jsonb) from public;
grant execute on function public.interval_advance_segment(uuid, jsonb) to authenticated;
grant execute on function public.interval_advance_segment(uuid, jsonb) to service_role;

-- ---------------------------------------------------------------------------
-- 4) RPC: amrap_reset_timer — also reset tabata/emom mechanics_state
-- ---------------------------------------------------------------------------
...
comment on function public.amrap_reset_timer(uuid) is
  'Host-only: resets timer to idle; AMRAP clears round events; tabata/emom resets mechanics_state template.';

-- ---------------------------------------------------------------------------
-- 5) RPC: interval_finalize_session — tabata effective_rounds + title
-- ---------------------------------------------------------------------------
      ...
      'session_telemetry', jsonb_build_object(
        ...
        'interval_performance', jsonb_build_array(
          jsonb_build_object(
            ...
            'rounds_target', (v_mechanics_state->>'total_rounds')::integer,
            'last_phase', 'finished',
