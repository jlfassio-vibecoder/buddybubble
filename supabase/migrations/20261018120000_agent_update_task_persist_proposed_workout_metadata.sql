-- Persist Coach `proposed_workout_metadata` on agent_update_task_and_reply reply metadata
-- so the client can sweep structural proposals onto the edit canvas (Phase 3).
...
comment on function public.agent_update_task_and_reply(
  ...
  'Agent updates an existing task + inserts thread reply with optional p_card_action, p_outline_draft_applied, p_workout_cues_patch, and p_proposed_workout_metadata on reply metadata; idempotent per trigger+agent; service_role only.';
