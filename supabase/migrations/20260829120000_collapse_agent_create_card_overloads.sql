-- Phase 12.1.2 — Collapse agent_create_card_and_reply to a single 15-arg overload.
--
-- `20260827120000_agent_rpcs_persist_card_action.sql` used `create or replace` with a
-- new `p_card_action` parameter, which added a second overload instead of replacing the
-- 14-arg function from `20260823120000_agent_rpcs_drop_legacy_overloads.sql`. PostgREST
-- then fails with "Could not choose the best candidate function" when optional JSONB args
-- (including `p_card_action`) are omitted — e.g. `insertSafeReply` on LLM timeout.
--
-- Drops every overload dynamically, then recreates the canonical body from
-- `20260828120000_fix_card_action_sibling_merge.sql`.
--
