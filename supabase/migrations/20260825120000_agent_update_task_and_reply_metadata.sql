-- Extend agent_update_task_and_reply for Live Co-Pilot Step 3: rail auto-apply of
-- proposed_workout_metadata into tasks.metadata (shallow jsonb merge), while
-- preserving title/description updates and all existing guard checks.
...
begin
  ...
  if v_title is null and v_desc is null and v_new_meta is null then
    raise exception 'agent_update_task_and_reply: at least one of p_new_title, p_new_description, or non-empty p_new_metadata must be provided'
      using errcode = 'P0001';
  end if;
...
comment on function public.agent_update_task_and_reply(
  ...
  'Agent updates an existing task (title, description, optional shallow metadata merge) + inserts thread reply (propagates thread_subject_user_id from trigger); idempotent per trigger+agent; service_role only.';
