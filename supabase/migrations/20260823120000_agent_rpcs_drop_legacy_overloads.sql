-- Phase 3.6A (C1): collapse all Postgres overloads of the Coach agent RPCs to a single
-- signature each (no zombie overloads). Prior migration 20260822120000 used `create or replace`
-- with a new typed parameter, which can leave a parallel legacy overload. This migration
-- drops every overload dynamically, then recreates the bodies from 20260822120000:
-- `agent_create_card_and_reply` (14 args) and `agent_insert_coach_workout_draft_reply` (12 args).
...
-- supabase/migrations/20260813120000_user_exercise_notes_and_personal_cues_rpc.sql:247-291
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Recreate: agent_create_card_and_reply (14-arg) — body from 20260822120000
-- ---------------------------------------------------------------------------
...
comment on function public.agent_create_card_and_reply(
  uuid, uuid, uuid, uuid, text, boolean, text, text, text, text, text, jsonb, jsonb, jsonb
) is
  'Agent reply with optional Kanban task; optional p_execution_patch, p_personal_cues, and p_task_modal_intake_patch on reply metadata; main reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
...
-- ---------------------------------------------------------------------------
-- Recreate: agent_insert_coach_workout_draft_reply (12-arg) — body from 20260822120000
-- ---------------------------------------------------------------------------
...
begin
  ...
  values (
    ...
    (
      ...
      || case
        when p_task_modal_intake_patch is not null
          and jsonb_typeof(p_task_modal_intake_patch) = 'object'
          and p_task_modal_intake_patch <> '{}'::jsonb
        then jsonb_build_object('task_modal_intake_patch', p_task_modal_intake_patch)
        else '{}'::jsonb
      end
    ),
    v_msg.thread_subject_user_id
...
comment on function public.agent_insert_coach_workout_draft_reply(
  ...
  'Agent inserts coach_draft reply; optional execution_patch, personal_cues_patch, and task_modal_intake_patch in metadata; reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';
