-- Realtime parity with PostgREST for `subject_threads` bubbles.
--
-- The `messages_select_subject_threads` policy
-- (see `20260728100000_message_thread_subject_rls.sql`) requires either
-- `thread_subject_user_id = auth.uid()` or workspace admin. Three V2 agent
-- RPCs insert the agent's main reply WITHOUT propagating the trigger
-- message's `thread_subject_user_id`, so non-admin subject viewers get the
-- row back via PostgREST (the seed-task-comment path was already correct in
-- `20260813120000_user_exercise_notes_and_personal_cues_rpc.sql`) but
-- realtime can drop the broadcast — the rail then needs a manual refresh
-- to surface the agent reply.
--
-- This migration re-creates the three functions with their current
-- behavior unchanged, adding only `thread_subject_user_id =
-- v_msg.thread_subject_user_id` on the main reply insert. Signatures are
-- preserved so existing GRANTs / `service_role` access carry over.
--
-- Functions touched:
...
begin
  ...
  -- Main agent reply: propagate `thread_subject_user_id` from the trigger
  -- message so `subject_threads` realtime delivery matches PostgREST RLS.
  insert into public.messages (
    bubble_id,
    user_id,
    content,
...
comment on function public.agent_create_card_and_reply(
  ...
  'Agent reply with optional Kanban task; optional p_execution_patch and p_personal_cues on reply metadata; main reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';

-- ---------------------------------------------------------------------------
-- 2. agent_insert_coach_workout_draft_reply
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Draft reply: propagate `thread_subject_user_id` from the trigger
  -- message so `subject_threads` realtime delivery matches PostgREST RLS.
  insert into public.messages (
    bubble_id,
    user_id,
...
comment on function public.agent_insert_coach_workout_draft_reply(
  ...
  'Agent inserts coach_draft reply; optional execution_patch and personal_cues_patch in metadata; reply propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity; applies cues to user_exercise_notes; service_role only.';

-- ---------------------------------------------------------------------------
-- 3. agent_update_task_and_reply
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Reply: propagate `thread_subject_user_id` from the trigger message so
  -- `subject_threads` realtime delivery matches PostgREST RLS.
  insert into public.messages (
    bubble_id,
    user_id,
    content,
...
comment on function public.agent_update_task_and_reply(
  ...
  'Agent updates an existing task in the bubble + inserts thread reply (propagates thread_subject_user_id from trigger for subject_threads RLS/realtime parity); idempotent per trigger+agent; service_role only.';
