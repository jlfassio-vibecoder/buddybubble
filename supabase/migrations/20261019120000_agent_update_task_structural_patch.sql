-- Add p_structural_patch (jsonb array) onto agent_update_task_and_reply reply metadata
-- for surgical Coach canvas field edits (block_id / exercise_id ops).
...
create or replace function public.agent_update_task_and_reply(
  ...
  p_structural_patch jsonb default null
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_structural_patch is not null
    and jsonb_typeof(p_structural_patch) = 'array'
    and jsonb_array_length(p_structural_patch) > 0
  then
    v_reply_meta := v_reply_meta || jsonb_build_object('structural_patch', p_structural_patch);
  end if;

  if v_title is null and v_desc is null and v_new_meta is null and v_reply_meta = '{}'::jsonb then
    raise exception 'agent_update_task_and_reply: at least one of p_new_title, p_new_description, non-empty p_new_metadata, p_card_action, p_outline_draft_applied, p_workout_cues_patch, p_proposed_workout_metadata, or p_structural_patch must be provided'
      using errcode = 'P0001';
  end if;
...
comment on function public.agent_update_task_and_reply(
  ...
  'Single signature: task title/description/metadata update + reply with optional card_action, outline_draft_applied, workout_cues_patch, proposed_workout_metadata, structural_patch; service_role only.';
