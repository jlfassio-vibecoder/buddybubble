-- Slack-style threads: agent reply parent_id = thread root (p_thread_id), not the trigger message id.
...
comment on function public.agent_create_card_and_reply(
  uuid, uuid, uuid, uuid, text, boolean, text, text, text, text
) is
  'Agent reply with optional Kanban task; p_thread_id = Slack thread root for parent_id; service_role only.';
