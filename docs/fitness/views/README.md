...
| **Data (read path — execution)**    | **WorkoutPlayer** (default modal) via `useWorkoutSessionViewModel`: block list, subtitles, **Tabata/EMOM interval shells**, alternating **modulo highlight routing**. **Active Session** (planned, opt-in route) shares the same ViewModel — see [active-session-engine-plan.md](../active-session-engine-plan.md). | Athletes see and **run** parametric intent, not only read it                                        |
...
**Bottom line:** The parametric engine is production-active in **WorkoutPlayer** (blocks, timers, alternating highlights). **Active Session** is a planned **side-by-side** route-based orchestration path—not a replacement. Remaining gaps are mostly **live-video flat loggers**, **block-aware editing** (Step 5), and advanced format UX (superset pairing, ladder progression UI).
