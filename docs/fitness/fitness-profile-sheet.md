...
Right-side **Sheet** for the per-user, per-workspace `**fitness_profiles`** row: goals, equipment checklist, unit system, biometrics (weight/height/age), and a **Quick workout\*\* action that calls the AI API to insert a workout card into the current bubble.
...
- **Load** (on `open`): `fitness_profiles` `select('*')` for `(workspace_id, user_id)`.
- **Biometrics display:** `decodeWeightDisplay` / `decodeHeightDisplay` convert stored `**weight_kg`** / `**height_cm`** to the user’s `unit_system`, with legacy handling for ambiguous `height`/`weight`keys (see inline comments — storefront historically wrote cm into`height` while labeling inches).
- **Save:** Builds `biometrics` JSON merging `**bioExtras`** (non-managed keys preserved) with managed keys: writes `**weight_kg`**, `**height_cm`**, `**age**`, strips legacy `weight`/`height` keys. `**insert**` vs `**update**` depends on whether a profile `id` exists.

## Quick workout

`POST /api/ai/quick-workout-from-profile` with JSON `{ workspace_id, bubble_id }`. On success shows a toast with returned `title` when present and invokes `onQuickWorkoutCreated`. If `bubbleIdForTasks` is missing, shows an error toast asking the user to pick a concrete bubble (not “All”).

## UX details

- **Goals** — free-text add list with dedupe.
- **Equipment** — toggles from `EQUIPMENT_OPTIONS` constant in source.
- **Unit system** — switching updates labels (kg/cm vs lbs/in) and how save encodes biometrics.

## Related docs

- [README.md](README.md)
- [workout-player.md](workout-player.md) (reads `unit_system` from the same table for display units during playback)
