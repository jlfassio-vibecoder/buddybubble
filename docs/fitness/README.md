...
- **Workout player:** `workoutPlayerTask` state holds a `TaskRow`; when set, [WorkoutPlayer](workout-player.md) opens with the task’s raw `metadata`, which the player parses internally. Started from `KanbanBoard` via `onStartWorkout` → `handleStartWorkout` (with trial gating).
