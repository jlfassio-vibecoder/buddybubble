...
| `apply_workout_draft`                    | Authenticated user (client)     | Merge `coach_draft` into the task; update draft state. Same migration file. **Known issue:** shallow JSONB `                                                                                                                                                                                                                 |     | `merge can drop nested enrichment on`tasks.metadata.exercises` — unrelated to personal cues; track as follow-up. |
