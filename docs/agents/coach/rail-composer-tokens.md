...
| `emom`          | Every minute on the minute      | `interval_seconds` + (`total_minutes` **or** `total_rounds`); optional `is_alternating` + `alternating_stations` (0-based index cycle). Catalog tokens `:phase/emom/alternating` set `is_alternating: true` without a matrix — parse hydrates `[[0],[1],…]` before validation. `:phase/emom/straight` and `:metcon/emom/density` set `is_alternating: false`. Legacy search aliases `pacing` / `threshold` still find alternating / density rows. **not** `circuit` for minute-bound alternating work |
...
- **EMOM taxonomy (Step 9):** `:main/emom/alternating `, `:main/emom/straight `, `:finisher/emom/alternating `, `:metcon/emom/density ` — third segment is picker/search metadata; persisted shape is `block_format: emom` + `format_params` (not the token string).
