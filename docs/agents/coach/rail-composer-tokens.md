...
| `emom`          | Every minute on the minute      | `interval_seconds` + (`total_minutes` **or** `total_rounds`); optional `is_alternating` + `alternating_stations` (0-based index cycle). Catalog tokens `:phase/emom/alternating` set `is_alternating: true` without a matrix — parse hydrates `[[0],[1],…]` before validation. `:phase/emom/alternating-combo` sets `is_combo: true` (catalog-only) — server hydrates A / B+C minutes `[[0],[1,2],…]` (last two exercises paired). `:phase/emom/straight` and `:metcon/emom/density` set `is_alternating: false`. Legacy search aliases `pacing` / `threshold` / `combo` still find alternating / density / combo rows. **not** `circuit` for minute-bound alternating work |
