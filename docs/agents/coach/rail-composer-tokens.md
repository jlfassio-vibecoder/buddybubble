...
**Interval exercise rows (`block_format: tabata`):** User-facing format label is **Intervals**; block subtitle uses preset labels (`Tabata · 8 Rounds (20/10s)` only for strict 20/10; `Classic HIIT · …` for 30/30, etc.). Merge hydrates each exercise via [`hydrateTabataExercisesFromFormatParams`](../../../src/lib/agents/_shared/workout-metadata/merge-coach-proposed-into-task-metadata.ts) (`rounds`, `workSeconds`, `restSeconds`; clears strength-style `sets`/`reps`). Applies to Lane 1, Lane 2, and Lane 3 so the viewer shows interval meta (e.g. `20s work · Rest 10s · 8 rounds`) on every movement card.
