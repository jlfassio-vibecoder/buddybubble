...
### Mention matching and interval block hydration
...
**Interval exercise rows (`block_format: tabata`):** User-facing format label is **Intervals**; block subtitle uses preset labels (`Tabata · 8 Rounds (20/10s)` only for strict 20/10; `Classic HIIT · …` for 30/30, etc.). For multi-exercise circuits, `format_params.rounds` is **circuit rounds** (passes through the exercise list); Coach must emit **N distinct `exercises[]` placeholders** when the user requests N movements (Phase F1). Timer work segments = `rounds × exerciseCount` (Phase F2). Merge hydrates each exercise via [`hydrateTabataExercisesFromFormatParams`](../../../src/lib/agents/_shared/workout-metadata/merge-coach-proposed-into-task-metadata.ts) (`rounds`, `workSeconds`, `restSeconds`; clears strength-style `sets`/`reps`). Applies to Lane 1, Lane 2, and Lane 3 so the viewer shows interval meta (e.g. `20s work · Rest 10s · 8 rounds`) on every movement card.
...
| `emom`          | Every minute on the minute                       | `interval_seconds` + (`total_minutes` **or** `total_rounds`); optional `is_alternating` + `alternating_stations` (0-based index cycle). Catalog tokens `:main/emom/alternating` (or `:finisher/emom/alternating`) set `is_alternating: true` without a matrix — parse hydrates `[[0],[1],…]` before validation. `:main/emom/alternating-combo` / `:finisher/emom/alternating-combo` set `is_combo: true` (catalog-only) — server hydrates A / B+C minutes `[[0],[1,2],…]` (last two exercises paired). `:main/emom/straight` and `:metcon/emom/density` set `is_alternating: false`. Legacy search aliases `pacing` / `threshold` / `combo` still find alternating / density / combo rows. **not** `circuit` for minute-bound alternating work |
| `tabata`        | Work/rest intervals (user-facing: **Intervals**) | `rounds` (optional `work_seconds`, `rest_seconds`, `interval_preset`). Preset ids: `tabata` (20/10/8 — **Tabata** label only here), `classic_hiit`, `hypertrophy_density`, `heavy_aerobic`, `power_sprints`, `fighters`, or `custom` when W/R diverge from a catalog preset. Catalog tokens `:finisher/hiit/classic `, `:main/interval/power `, etc. set all four keys; legacy `:finisher/tabata/vo2 ` / `:main/tabata/power ` remain strict Tabata 20/10/8. Coach must not say "Tabata-style" for non-20/10. See §5.5.                                                                                                                                                                                                                        |
...
- **HIIT** is not a separate enum value — map to **`emom`**, **`tabata`** (user-facing **Intervals**), or **`amrap`** in copy/examples (or **`chipper`** when the user wants sequential for-time work). Say **Tabata** only for strict 20/10/8; use preset names (Classic HIIT, etc.) for other work/rest ratios.
...
- **EMOM taxonomy (Step 9):** `:main/emom/alternating `, `:main/emom/alternating-combo `, `:main/emom/straight `, `:finisher/emom/alternating `, `:finisher/emom/alternating-combo `, `:metcon/emom/density ` — third segment is picker/search metadata; persisted shape is `block_format: emom` + `format_params` (not the token string; `is_combo` is stripped at hydrate).
- **Interval preset taxonomy (work/rest):** `:finisher/hiit/classic ` (30/30 × 8), `:finisher/hiit/density ` (40/20 × 8), `:cardio/hiit/aerobic ` (60/60 × 6), `:main/interval/power ` (10/50 × 6), `:main/interval/fighters ` (5:00 / 1:00 × 5) — middle segment (`hiit`, `interval`) is search metadata only; persisted shape is always `block_format: tabata` + `format_params` including `interval_preset`. Legacy tabata tokens (`:finisher/tabata/vo2 `, `:main/tabata/power `, `:cardio/tabata/vo2 `) remain Tabata 20/10/8.

### 5.6 Block catalog (flat + fuzzy search)

**Source of truth:** [`src/lib/agents/coach/block-blueprint-catalog.ts`](../../../src/lib/agents/coach/block-blueprint-catalog.ts) (~43 pre-combined rows; grows via PR).
...
Example: `:finisher/tabata/vo2 ` or `:finisher/hiit/classic ` — the third segment (`vo2`, `classic`, `core`) is **search and grouping metadata only**. Lane 1/2/3 still read structured fields on send:

- `section_name` (e.g. `Finisher`)
- `block_format` (closed-world enum, e.g. `tabata` even when token segment says `hiit`)
- `format_params` (defaults from catalog row, including `interval_preset` when applicable)
...
**Phase B (shipped):** `ladder` and `chipper`. **Phase C (shipped):** `pyramid`, `contrast`, `clusters`, `drop_sets` catalog rows and engine support; **interval preset rows** (`:finisher/hiit/*`, `:main/interval/*`, `:cardio/hiit/aerobic`) for industry work/rest ratios. **Deferred:** instruction-only mobility/LISS without `block_format`.
