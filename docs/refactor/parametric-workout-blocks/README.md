...
| **Creation / pre-rich draft context**      | When emitting `proposed_workout_metadata.blocks`, every **exercise-shaped** block MUST include `block_format` + `format_params` per BLUEPRINT LIBRARY; instruction-only warm-up/cool-down blocks are exempt. | Full proposed metadata is creation-only. Existing rich rail canvases use `structural_patch`, with the creation field pruned from their schema. |
