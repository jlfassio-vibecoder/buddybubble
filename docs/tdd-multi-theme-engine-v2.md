...
| **Category palette** | **Primary source:** `workspace.category_type` (dashboard; route-resolved + SSR seed) or invite preview `category_type` (`/invite/[token]`). **Optional client override (per BuddyBubble):** `buddybubble.categoryThemeOverride.{workspaceId}` in `localStorage` (`auto` or a fixed `WorkspaceCategory`) via Profile → appearance for the **current** socialspace; when not `auto`, that category wins for **paint only** (`ThemeScope` / portaled Dialog·Sheet). Product behavior (Kanban fitness chrome, TaskModal date labels, family names) always uses DB `category_type`. Invite pages have no workspace yet, so they **only** use RPC `category_type`. The retired global key `bb_category_theme_override` is migrated once into the first workspace that reads it, then deleted. |

**Merge rule:** `next-themes` answers “render light or dark”; **theme category** answers “which registry row to paint” (workspace/invite row, unless the user chose a preview override); the theme engine **merges** both into one variable map. **Workspace category** (DB) stays separate for feature gates and copy.
...
| Category source              | **Database / RPC** (`category_type`) as default; **optional** per-workspace client paint override via `buddybubble.categoryThemeOverride.{workspaceId}` (see §7). |
