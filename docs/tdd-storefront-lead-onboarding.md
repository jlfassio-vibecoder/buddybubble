...
| CTA island        | [`apps/storefront/src/components/hero/StorefrontHero.jsx`](../apps/storefront/src/components/hero/StorefrontHero.jsx) (phase components under [`hero/phases/`](../apps/storefront/src/components/hero/phases/)) | **Phase S1:** “Start 3-Day Preview” opens a **multi-step hero** (profile → outline preview → fitness refine → **email gate**) → `fetch('/api/storefront-trial', …)` with `publicSlug`, `email`, `source`, `utmParams`, **`profile`**. Draft state in **`sessionStorage`** key `buddybubble_storefront_trial_v1:{slug}` — **no** CRM calls until email submit. |
...
4. **Hero CTA** — Prefer a single primary (“Start 3-day preview” / “Build your plan”) that **enters** the `StorefrontHero` flow; **email only on the final step**.
...
| **S1**  | Storefront **multi-step** profile + **sessionStorage**; **email last**; pass **`profile`** + existing `source` / `utmParams` to proxy → CRM | **Shipped** (`StorefrontHero` + `buddybubble_storefront_trial_v1:{slug}` persistence) |
